...
struct CodeContext
{
    ...
    std::map<std::string, int> stackIndexMap; //variables/arrays -> indices on the stack
    
    int nextReg = 0; //
    int stackSize = 0;
    
    //Add operations and its params
    void AddOp(std::string op, std::vector<std::string> param){
        ops.push_back(op);
        params.push_back(param);
    }
    
    //Allocate the declaration on stack and store the indices for later used
    void AddStack(std::string name, int size) {
        stackIndexMap[name] = stackSize;
        ++stackSize;
    }
    
    //Get and Update the next available register
    std::string GetReg(){
        return "%"+std::to_string(nextReg++);

    }
    
    //Print the operations and its params to emit.txt
    void OutputEmit(){
        std::ofstream ofile("emit.txt");
